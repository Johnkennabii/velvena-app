...
export default function DressTypes() {
  const [types, setTypes] = useState<DressType[]>([]);
  ...
  const [confirmState, setConfirmState] = useState<ConfirmState>({ mode: "soft", type: null });
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        setLoading(true);
        const data = await DressTypesAPI.list();
        setTypes(data);
      } catch (error) {
        console.error("❌ Impossible de charger les types de robe :", error);
        notify("error", "Erreur", "Le chargement des types a échoué.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const openEditModal = (row: TypeRow) => {
    setEditType(row);
    setEditForm({
      name: row.name,
  ...
  const requestSoftDelete = (row: TypeRow) => {
    if (!isAdmin && !isManager) {
      notify("warning", "Action non autorisée", "Vous n'avez pas les droits suffisants.");
      return;
    }
    setConfirmState({ mode: "soft", type: row });
  };

  const requestHardDelete = (row: TypeRow) => {
    if (!isAdmin) {
      notify("warning", "Action non autorisée", "Seul un administrateur peut supprimer définitivement un type.");
      return;
    }
    setConfirmState({ mode: "hard", type: row });
  };

  const resetConfirm = () => setConfirmState({ mode: "soft", type: null });

  const performSoftDelete = async (row: TypeRow) => {
    setProcessing({ type: "soft", id: row.id });
    let success = false;
    try {
      await DressTypesAPI.softDelete(row.id);
      setTypes((prev) => prev.filter((item) => item.id !== row.id));
      notify("success", "Type désactivé", "Le type de robe a été désactivé.");
      success = true;
    } catch (error) {
      console.error("❌ Soft delete type :", error);
      notify("error", "Erreur", "Impossible de désactiver le type.");
    } finally {
  ...
  const performHardDelete = async (row: TypeRow) => {
    setProcessing({ type: "hard", id: row.id });
    let success = false;
    try {
      await DressTypesAPI.hardDelete(row.id);
      setTypes((prev) => prev.filter((item) => item.id !== row.id));
      notify("success", "Type supprimé", "Le type de robe a été supprimé définitivement.");
      success = true;
    } catch (error) {
      console.error("❌ Hard delete type :", error);
      notify("error", "Erreur", "Impossible de supprimer le type.");
    } finally {
      setProcessing({ type: null, id: null });
    }
    return success;
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const created = await DressTypesAPI.create(payload);
      setTypes((prev) => [created, ...prev]);
      notify("success", "Type créé", "Le type de robe a été ajouté.");
      setCreateOpen(false);
    } catch (error) {
      console.error("❌ Création type :", error);
      notify("error", "Erreur", "Impossible de créer le type.");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updated = await DressTypesAPI.update(editType.id, payload);
      setTypes((prev) => prev.map((type) => (type.id === editType.id ? updated : type)));
      notify("success", "Type mis à jour", "Les modifications ont été enregistrées.");
      setEditType(null);
    } catch (error) {
      console.error("❌ Mise à jour type :", error);
      notify("error", "Erreur", "Impossible de mettre à jour le type.");
    } finally {
      setUpdating(false);
    }
  };

  const confirmType = confirmState.type;
  const confirmLoading =
    !!confirmType && processing.type === confirmState.mode && processing.id === confirmType.id;
  const confirmTitle =
    confirmState.mode === "soft" ? "Désactiver le type" : "Supprimer le type";
  const confirmDescription =
    confirmState.mode === "soft"
      ? "Cette action désactivera temporairement le type de robe."
      : "Cette action est définitive. Toutes les données associées à ce type seront supprimées.";
  const confirmAccent =
    confirmState.mode === "soft"
  ...
  return (
    <>
      <PageMeta title="Types de robe - Velvena App" description="Administration des types de robe." />
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="flex flex-col gap-3 border-b border-gray-200 px-6 py-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Types de robe</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {loading ? "Chargement..." : `${rows.length} type${rows.length > 1 ? "s" : ""}`}
            </p>
          </div>
          {(isAdmin || isManager) && (
        ...
        {loading ? (
          ...
          <div className="flex w-full flex-col items-center justify-center gap-2 py-16 text-center">
            <h4 className="text-base font-medium text-gray-800 dark:text-white/90">Aucun type disponible</h4>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Dès qu’un type sera créé, il apparaîtra automatiquement ici.
            </p>
          </div>
        ) : (
        ...
        <div className="flex flex-col gap-6">
          ...
          {confirmType && (
            <div className={`rounded-xl border px-4 py-3 text-sm ${confirmAccent}`}>
              <p>
                Type :<span className="font-semibold"> {confirmType.name}</span>
              </p>
              {confirmType.description ? (
                <p className="mt-1 text-xs text-gray-600 dark:text-gray-400">{confirmType.descriptionPreview}</p>
              ) : null}
            </div>
          )}
        ...
        <form className="flex flex-col gap-6" onSubmit={handleCreate}>
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              Ajouter un type de robe
            </h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Complétez les informations suivantes pour créer un type.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <TextArea
                ...
                placeholder="Décrivez brièvement ce type de robe"
              />
            </div>
          </div>
      ...
      >
        <form className="flex flex-col gap-6" onSubmit={handleUpdate}>
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              Modifier le type de robe
            </h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Ajustez les informations du type sélectionné.
            </p>
          </div>
