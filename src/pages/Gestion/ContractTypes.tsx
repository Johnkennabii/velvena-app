...
interface TypeRow extends ContractType {
  createdLabel: string;
  updatedLabel: string;
}
...
export default function ContractTypes() {
  ...
  const performSoftDelete = async (row: TypeRow) => {
    ...
    let success = false;
    try {
      await ContractTypesAPI.softDelete(row.id);
      setTypes((prev) => prev.filter((item) => item.id !== row.id));
      notify("success", "Type désactivé", "Le type de contrat a été désactivé.");
      success = true;
    } catch (error) {
  ...
  const performHardDelete = async (row: TypeRow) => {
    ...
    let success = false;
    try {
      await ContractTypesAPI.hardDelete(row.id);
      setTypes((prev) => prev.filter((item) => item.id !== row.id));
      notify("success", "Type supprimé", "Le type de contrat a été supprimé définitivement.");
      success = true;
    } catch (error) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      setCreating(true);
      const created = await ContractTypesAPI.create({ name: createName.trim() });
      setTypes((prev) => [created, ...prev]);
      notify("success", "Type créé", "Le type de contrat a été ajouté.");
      setCreateOpen(false);
    } catch (error) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setUpdating(true);
      const updated = await ContractTypesAPI.update(editType.id, { name: editName.trim() });
  ...
  const confirmDescription =
    confirmState.mode === "soft"
      ? "Cette action désactivera temporairement le type de contrat."
    ...
    <>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="flex flex-col gap-3 border-b border-gray-200 px-6 py-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Types de contrat</h3>
                  ...
                  <TableRow key={row.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.05]">
                    ...
                    <TableCell className="px-4 py-3">
                      <Badge variant="light" color={row.deleted_at ? "warning" : "success"} size="sm">
                        {row.deleted_at ? "Désactivé" : "Actif"}
                      </Badge>
                    </TableCell>
                    <TableCell className="px-4 py-3">
          ...
          {confirmType && (
            <div className={`rounded-xl border px-4 py-3 text-sm ${confirmAccent}`}>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400">
                Créé le {confirmType.createdLabel}
              </p>
            </div>
          )}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Ex : Location longue durée"
                required
              />
            </div>
          </div>
