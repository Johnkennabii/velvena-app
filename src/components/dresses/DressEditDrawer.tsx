...
export interface DressEditDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  loading: boolean;
  form: DressFormState | null;
...
export function DressEditDrawer({
  isOpen,
  onClose,
  loading,
  form,
  ...
  dressColors,
  maxImages,
}: DressEditDrawerProps) {
  return (
    <RightDrawer
      isOpen={isOpen}
      onClose={onClose}
      title="Modifier la robe"
      description="Mettre à jour les informations de la robe"
        ...
        <form className="space-y-6" onSubmit={onSubmit}>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label>Images ({form.images.length}/{maxImages})</Label>
              {uploadInProgress ? (
          ...
          <div className="flex justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-800">
            <Button type="button" variant="outline" onClick={onClose} disabled={uploadInProgress}>
              Annuler
            </Button>
